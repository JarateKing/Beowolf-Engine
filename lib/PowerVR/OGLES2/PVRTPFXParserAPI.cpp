...
EPVRTError CPVRTPFXEffect::Activate(const int i32RenderTextureId, const unsigned int ui32ReplacementTexture)
{
	...
	{
		uiTextureId = m_psTextures[m_pnTextureIdx[i]].ui;
		...
		if((psParserEffect->psTextures[i].u32Type&PVRTEX_CUBEMAP)!=0)
			glBindTexture(GL_TEXTURE_CUBE_MAP, psParserEffect->psTextures[i].uTextureHandle);
		else
			glBindTexture(GL_TEXTURE_2D, psParserEffect->psTextures[i].uTextureHandle);
	}
...
{
	nCount = m_pParser->m_psTexture.GetSize();
	return m_psTextures;
}
...
{
	if(nIdx < (unsigned int) m_pParser->m_psTexture.GetSize())
	{
		...
		// store flags
		unsigned int uiIndex = m_pParser->FindTextureIndex(CPVRTString(m_pParser->m_psTexture[nIdx].pszName), m_nEffect);
		if(uiIndex != 0xFFFFFFFF)
		{
			m_pParser->m_psEffect[m_nEffect].psTextures[uiIndex].u32Type = u32flags;
			m_pParser->m_psEffect[m_nEffect].psTextures[uiIndex].uTextureHandle = ui;
		}
	}
}
